...
//        preg_match_all("/(?<=\").*?(?=\")/s", $receipt, $result);
//
//        $receipt_info = $result[0][6];
...
//        $cache_key = 'log_step2';
//        $time = 60 * 60;
//        Cache::add($cache_key, json_encode($receipt_info), $time);
        ...
        // transfer to game
        if ($response = $service->transfer($uid, $server_id, $trans_id, $price, $ip, $inreview_status)) {
            //if (true) {
            $cpid = UtilHelper::getDefaultCpid(2, $client_id); // fix 2 - ios
