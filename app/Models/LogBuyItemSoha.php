...
class LogBuyItemSoha extends Model {
     ...
     * @var bool
     */
    protected $fillable = ['userid', 'soha_order_info', 'cpid'];
    ...
    public static function getLogBuySohaTransid($order_info, $status) {
        if (!empty($order_info) && !empty($status)) {
            return static::where('soha_order_info', $order_info)
                            ->where('status', $status)
                            ->firstOrFail();
        }
        return new static;
