...
class Dau_log extends Model {

    //
    protected $table = 'dau_log';

    public function show_week($cpid) {
        $log = DB::table('dau_log')
...
//                    ->where('merchant_app_cp.app_id', '=', $id)
    ...
    public static function list_cpid($app_id, $partner_id, $user_type) {
        $app_id_cp = array();
        $parner_id_cp = array();
        foreach ($app_id as $key => $app_id) {
            $app_id_cp[] = $app_id->app_id;
        }
        ...
        if($user_type =='admin'){
            ...
            $list_cpid_querry = Merchant_app_cp::
                    wherein('app_id',  $app_id_cp)
                    ->wherein('partner_id', $parner_id_cp)
                    ->get();
        }
        return $list_cpid_querry;
    }
    public function get_cp_name($cpid){
