@extends('app')
@section('contentheader_title', 'Danh sách dau')
@section('css-current')
            ...
            <div class="row" style="margin: 0 0 0 5px;">
                <div class="box-tools pull-right col-md-2">
                    @if(isset($_GET['date-from']))
                        <a class="btn btn-primary" href="<?php echo "//".$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];  ?>&xlsexport=reportdau_from<?php echo $datefromday.'-'.$datetoday?>" role="button">Export to excel</a>
                    @endif
                </div>
            </div>
            <?php } ?>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    @foreach ($result['data'] as $row)<td>{{$row}}</td>@endforeach
                    <td>{{max($result['data'])}}</td>
                    <td>{{round(array_sum($result['data']) / count($result['data']),2)}}</td>
                </tr>
                @endforeach
...
$(function () {
    ...
    $('#chartmonitor').highcharts({
        ...
        xAxis: {
            categories: [@foreach ($xcols as $key => $row)'{{$row}}',@endforeach]
        },
        yAxis: {
            title: {
                text: 'dau'
        ...
        series: [
            @foreach ($results as $key => $result)
            {
                name: '<?php echo $results[$key]["name"]; ?>',
...
//            if($('.cp_id:checked ').length >= 1){
//                if (!$('#date-from').is(":empty")&&!$('#date-to').is(":empty")) {
//                    $( "#logniu" ).submit();
//                }
//            }else{
//                alert("Cần chọn ít nhất một cp để hiển thị log doanh thu !!!");
