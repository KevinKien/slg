@extends('app')
@section('contentheader_title', 'Danh sách NIU')
@section('css-current')
    ...
    ?>
            ...
            <div class="box-tools pull-right col-md-2">
                    @if(isset($_GET['date-from']))
                        <a class="btn btn-primary" href="<?php echo "//".$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];  ?>&xlsexport=reportniu_from<?php echo $datefromday.'-'.$datetoday?>" role="button">Export to excel</a>
                    @endif
            </div>
            </div>
